...
def main():
    ...
    if gap.empty:
        print("錯誤: return gap 資料為空")
        return
    print(f"搜尋最佳化參數(K範圍: {K_RATIO_LIST[0]} ~ {K_RATIO_LIST[-1]})...")
    results = []
    for k in tqdm(K_RATIO_LIST):
        gia = compute_gia(gap, holding, k_ratio = k)
        if gia.empty:
            continue
        wide, summary = backtest_single_decile(gia, stock_q, entry_elig, gia_col='GIA', n_groups=10, nw_lags=NW_LAGS)
    ...
    df_res = pd.DataFrame(results).sort_values(by = ['mean', 't', 'rho', 'viol'], ascending = [True, True, True, True])
    ...
    print("="*50 + "\n")
    print("進行最終回測...")
    final_gia = compute_gia(gap, holding, k_ratio = best_k)

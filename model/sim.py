...
def compute_carhart4_skill(fund_monthly_df: pd.DataFrame, factor_df: pd.DataFrame, window = WINDOW):
    ...
    results = []
    data = pd.merge(fund_monthly_df, factor_df, on = '年月', how = 'inner')
    x_cols = ['MKT', 'SMB', 'HML', 'MOM']
    for fund, group in tqdm(data.groupby('基金代碼'), desc = '計算基金alpha'):
        qe_dates = group[group['年月'].dt.is_quarter_end]['年月']
        for qe in qe_dates:
            mask = group['年月'] <= qe
            win_data = group[mask].tail(window)
            if len(win_data) < 10:
                continue
            Y = win_data['ret_month'] - win_data['RF']
            X = sm.add_constant(win_data[x_cols])
            try:
                ...
                print(f'基金 {fund} 在 {qe} 的回歸失敗: {e}')
                continue
    return pd.DataFrame(results)


def compute_sim_skill(holding_df: pd.DataFrame, alpha_df: pd.DataFrame):
    '''計算基金的持倉相似度指標，當作基金 alpha 的 proxy'''
    print("正在計算基金 持倉相似度 指標")
    merged = pd.merge(holding_df, alpha_df, on = ['基金代碼', '年季'], how = 'inner')
    merged['w_times_alpha'] = merged['past_alpha'] * merged['w']
    stock_stats = merged.groupby(['年季', 'key_code']).agg(
        sum_w_alpha = ('w_times_alpha', 'sum'),
        sum_w = ('w', 'sum')
    ).reset_index()
    stock_stats['delta_quality'] = stock_stats['sum_w_alpha'] / stock_stats['sum_w'].replace(0, np.nan)
    final_df = pd.merge(holding_df, stock_stats[['年季', 'key_code', 'delta_quality']], on = ['年季', 'key_code'], how = 'left')
    final_df['delta_quality'] = final_df['delta_quality'].fillna(0)
    sim_skill = final_df.groupby(['基金代碼', '年季']).apply(
        lambda x: np.sum(x['w'] * x['delta_quality'])
...
def main():
    ...
    print("計算 SIM alpha")
    fund_carhart_alpha = compute_carhart4_skill(fund_m, factor_data, window = WINDOW)
    fund_skill = compute_sim_skill(holding, fund_carhart_alpha) 

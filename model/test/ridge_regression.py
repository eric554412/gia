...
def run_ml_stock_prediction_ridge_cv(holding_w, stock_q, ridge_alphas, min_funds=10, lookback_quarters=2):
    ...
    # 1. 資料合併 (注意：傳入的 stock_q 已經是 Shift 過的)
    ...
    for i in range(start_idx, len(unique_quarters)):
        ...
        X_target, _ = target_data # 只需要 X (預測用)
...
# =========================
def main():
    print(f"=== 啟動 ML RidgeCV (Predictive Model, Shift=1) ===")
    ...
    # =========================================================
    # 🚨 關鍵動作：建立 ML 專用的平移資料 (stock_q_ml)
    # 邏輯：ML 訓練時，T 季的持股 (X) 對應 T+1 季的報酬 (Y)
    # =========================================================
    print("\n[系統校正] 建立 ML 訓練目標 (Y) 的 Lag 1 平移...")
    stock_q_ml = stock_q.copy()
    stock_q_ml['年季'] = stock_q_ml['年季'] - 1
    ...
    print(f"  > Strategy    : Predictive (X_t -> Y_t+1)")

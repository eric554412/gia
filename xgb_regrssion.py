...
# =========================
# 3. 核心運算: XGBoost CV (原始 Y)
# =========================
def run_ml_stock_prediction_xgboost_cv(holding_w, stock_q, param_grid, fixed_params, min_funds=10, lookback_quarters=2):
    ...
    print(f"      > ML XGBoost Lite (Lookback={lookback_quarters}, {CV_FOLDS}-Fold CV)")
    ...
    for i in tqdm(range(start_idx, len(unique_quarters)), desc="滾動回測進度", unit="Q"):
        ...
        Y_train = pd.concat(Y_train_list) # [修改] 直接使用原始報酬，不截斷
        
        X_target, Y_true_target = target_data 
        
        n_samples = len(X_target)
        
        all_cols = X_train.columns
        X_target = X_target.reindex(columns=all_cols, fill_value=0)
        ...
        try:
            ...
            y_pred_target = best_model.predict(X_target)
            
            # 計算 Metrics
            mse_model = np.mean((Y_true_target - y_pred_target)**2)
            mse_zero = np.mean(Y_true_target**2)
            r2_oos = 1 - (mse_model / mse_zero)
            
            ic_pearson = np.corrcoef(Y_true_target.values.flatten(), y_pred_target.flatten())[0, 1]
            ic_spearman = pd.Series(Y_true_target.values.flatten()).corr(pd.Series(y_pred_target.flatten()), method='spearman')
            ...
            tqdm.write(f"Error at {target_q}: {e}")
            continue
    ...
    return {'Rolling_Best': combined_df}, metrics_df
...
def backtest_single_decile(gia_df, qret_df, eligibility_df, score_col, n_group=10, nw_lags=NW_LAGS):
    ...
    g_grp = g_grp.rename(columns={'formation_q': 'formation_q'})
...
# =========================
def main():
    print(f"=== 啟動 ML XGBoost Lite (Fast CV + No Winsorization) ===")
    
    n_params = 1
    for k, v in XGB_PARAM_GRID.items():
        n_params *= len(v)
    print(f"參數網格組合數: {n_params} 種")
    ...
    # 1. 執行機器學習預測
    all_pred_results, metrics_df = run_ml_stock_prediction_xgboost_cv(
    ...
    # --- 顯示 R2, IC 和 N 統計 ---
    print("\n" + "="*60)
    print(f"=== 模型預測能力統計 (Model Accuracy Metrics) ===")
    print("="*60)
    if not metrics_df.empty:
        print(f"平均樣本數 (N)  : {int(metrics_df['N_Stocks'].mean())}      (廣度證明)")
        print(f"平均 R2 (OOS)   : {metrics_df['R2_OOS (%)'].mean():.4f}%")
        print(f"平均 IC (Pearson): {metrics_df['IC_Pearson'].mean():.4f}")
        print(f"平均 IC (Rank)   : {metrics_df['IC_Rank'].mean():.4f}")
        print("-" * 60)
        print("最近 5 季詳細數據:")
        print(metrics_df[['年季', 'N_Stocks', 'R2_OOS (%)', 'IC_Pearson', 'IC_Rank']].tail(5).to_string(index=False))
    else:
        print("無 Metrics 數據")
    print("="*60)
